...
local omi_siac_cqs_snapshot_cta_v2_1 = Proto("Omi.Siac.Cqs.Snapshot.Cta.v2.1", "Siac Cqs Snapshot Cta 2.1")
...
-- Snapshot Message
siac_cqs_snapshot_cta_v2_1.snapshot_message = {}

-- Calculate size of: Snapshot Message
siac_cqs_snapshot_cta_v2_1.snapshot_message.size = function(buffer, offset)
  local index = 0
...
siac_cqs_snapshot_cta_v2_1.snapshot_message.display = function(packet, parent, length)
  return ""
end

-- Dissect Fields: Snapshot Message
siac_cqs_snapshot_cta_v2_1.snapshot_message.fields = function(buffer, offset, packet, parent)
  local index = offset

  -- Snapshot Message Type: 1 Byte Ascii String
...
siac_cqs_snapshot_cta_v2_1.snapshot_message.dissect = function(buffer, offset, packet, parent)
  if show.structs then
    -- Optionally add element to protocol tree
    parent = parent:add(omi_siac_cqs_snapshot_cta_v2_1.fields.snapshot_message, buffer(offset, 0))
    local index = siac_cqs_snapshot_cta_v2_1.snapshot_message.fields(buffer, offset, packet, parent)
    local length = index - offset
    parent:set_len(length)
    local display = siac_cqs_snapshot_cta_v2_1.snapshot_message.display(packet, parent, length)
    parent:append_text(display)

    return index, parent
  else
    -- Skip element, add fields directly
    return siac_cqs_snapshot_cta_v2_1.snapshot_message.fields(buffer, offset, packet, parent)
  end
end
...
siac_cqs_snapshot_cta_v2_1.category_payload.dissect = function(buffer, offset, packet, parent, message_category)
  ...
  if message_category == "R" then
    return siac_cqs_snapshot_cta_v2_1.snapshot_message.dissect(buffer, offset, packet, parent)
  end
...
-- Register Siac Cqs Snapshot Cta 2.1 for Decode As
local udp_table = DissectorTable.get("udp.port")
udp_table:add_for_decode_as(omi_siac_cqs_snapshot_cta_v2_1)
